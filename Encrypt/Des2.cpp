...
void  DES_Efun(char* input, int in_length, char* key_in, char* Out)
{
	char text[8];
	memcpy(text, input, in_length);
	if (in_length < 8) {
		memset(&text[in_length], 8 - in_length, 8 - in_length);
	}
	...
	int key[64];
	int l[17][32], r[17][32];
	CharToBit(text, chartobit, 8);//正确,转换为64个二进制数的操作正确！  
	...
	IP_In(output_1, Mingwen, IPR_Table);
